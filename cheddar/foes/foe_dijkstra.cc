...
struct _QItem {
  ...
  bool operator<(const _QItem& other) const { return this->dist < other.dist; }
};
  ...
  for (int _y = y - 1; _y <= y + 1; _y++) {
    for (int _x = x - 1; _x <= x + 1; _x++) {
      ...
      neighbours.push_back(_Neighbour{
          ...
          (_x != x &&
           _y != y)  // neighbour is diagonal if neither x and y are in common
      });
    }
  }
}
...
bool foe_move_towards(Mouse* mouse, int* x, int* y, FoeStrafe strafe) {
  ...
  *y = i / rows;
  ...
  if (strafe == FoeStrafe::NO_STRAFE) {
    switch (SDL_rand(4)) {
      case 0:
        strafe = FoeStrafe::STRAFE_LEFT;
        break;
      case 1:
        strafe = FoeStrafe::STRAFE_RIGHT;
        break;
      default:
        break;
    }
  ...
  if (strafe != FoeStrafe::NO_STRAFE && SDL_rand(2) == 1) {
    ...
    if (strafe_x >= 0 && strafe_x < cols && strafe_y >= 0 && strafe_y < rows &&
        !COLLISION(strafe_x, strafe_y)) {
      *x = strafe_x;
      *y = strafe_y;
...
bool foe_move_away(Mouse* mouse, int* x, int* y) {
  ...
  if (away_x < 0 || away_x >= cols || away_y < 0 || away_y >= rows ||
      COLLISION(away_x, away_y)) {
    return false;
  }
...
static SDL_FRect debug_src_rect[] = {
    SDL_FRect{0.0f, 0.0f, 16.0f, 16.0f},  SDL_FRect{16.0f, 0.0f, 16.0f, 16.0f},
    SDL_FRect{32.0f, 0.0f, 16.0f, 16.0f}, SDL_FRect{48.0f, 0.0f, 16.0f, 16.0f},
    SDL_FRect{64.0f, 0.0f, 16.0f, 16.0f}, SDL_FRect{80.0f, 0.0f, 16.0f, 16.0f},
    SDL_FRect{96.0f, 0.0f, 16.0f, 16.0f}, SDL_FRect{112.0f, 0.0f, 16.0f, 16.0f},
