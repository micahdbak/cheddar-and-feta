...
void FoePorcupine::action(Mouse* mouse) {
  ...
  dir_to_point(this->x, this->y, mouse->x, mouse->y, &this->spine_x_dir,
               &this->spine_y_dir);
  this->sprite->set_animation(
      ATTACK_ANIMATION +
      direction_from_dirs(this->spine_x_dir, this->spine_y_dir));
...
void FoePorcupine::step() {
  ...
  switch (this->state) {
    case Foe::State::ACTION:
      ...
      } else if (game->ticks - this->timer > 400 &&
    ...
    case Foe::State::THROWN:
      if (this->prev_state != this->state) {
        play_audio("sfx/ant_hurt.wav", 1.0f, this->x, this->y, false);
      }
    ...
    case Foe::State::DEAD:
      if (this->prev_state != this->state) {
        play_audio("sfx/ant_die.wav", 1.0f, this->x, this->y, false);
      }
  ...
  if (game->ticks - this->hurt_timer < 250 && this->state != Foe::State::DEAD) {
  ...
  if (this->sprite->update_frame() && this->state == Foe::State::WALKING &&
    ...
    play_audio("sfx/step.wav", 1.0f, this->x, this->y, false);
  }
