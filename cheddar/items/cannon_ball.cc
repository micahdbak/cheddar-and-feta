...
ThrownCannonBall::ThrownCannonBall(float x, float y, int x_dir, int y_dir, int from_id)
    ...
    this->sprite = new Sprite("sprites/item_cannon_ball.bmp", 16, 16, 0);
...
void ThrownCannonBall::step() {
    float dx = float(this->x_dir) * (this->y_dir != 0 ? DIAG_MULTIPLIER : 1.0f) * 64.0f * game->delta;
    float dy = float(this->y_dir) * (this->x_dir != 0 ? DIAG_MULTIPLIER : 1.0f) * 64.0f * game->delta;
    ...
    if (game->point_in_collider(new_x, new_y) || game->ticks - this->spawned_ticks > 750) {
        ...
        if (this->drop_item) {
            game->push_object(ITEM_CANNON_BALL DROPPED_OBJ, DroppedItem::Options(this->x, this->y));
        }
