...
void LoadSave::step() {
  if (textbox != nullptr) {
    ...
    if (!save.has(LOAD_MAP)) save.data[LOAD_MAP] = "maps/demo00";

    game->map = save.data[LOAD_MAP];
    return;
  }

  int diff = local_controller.is_hit(Button::DOWN) -
  ...
  if (this->render) {
    ...
    for (int i = save_start; i <= save_end; i++) {
      ...
      game->draw_ui_box(game->ui, this->sel_save == i ? BOX_OUT_SEL : BOX_OUT,
                        &highlight_rect);
      std::string save_text =
          i >= this->summaries.size() ? NEW_SAVE_STR : this->summaries[i];
      save_text = std::to_string(i + 1) + ": " + save_text;
      game->draw_text(game->ui, save_text, DEFAULT_FONT, x + PADDING + 2,
                      save_y + 3, 0);
    }

    if (save_start > 0) {
      game->draw_text(game->ui, "^", SMALL_FONT, (SCREEN_WIDTH / 2) - 3, y + 3,
                      0);
    }

    if (save_end < this->summaries.size()) {
      game->draw_text(game->ui, "}", SMALL_FONT, (SCREEN_WIDTH / 2) - 3,
                      y + UI_HEIGHT - 9, 0);
    }
