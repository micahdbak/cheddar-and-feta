...
Mouse* Mouse::closest_mouse(float x, float y, float min_distance, bool forced) {
  float distance_cheddar =
      (cheddar->is_down && !forced)
          ? FLT_MAX
          : distance_between_points(x, y, cheddar->x, cheddar->y);
  float distance_feta = (feta->is_down && !forced)
                            ? FLT_MAX
                            : distance_between_points(x, y, feta->x, feta->y);
