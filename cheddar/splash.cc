...
void Splash::step() {
  ...
  if (this->animation == 4) {
    ...
    SDL_FRect tile_dst = {float(SCREEN_WIDTH / 2 - 32), 0.0f - perc * 128.0f,
                          64.0f, (float)SCREEN_HEIGHT + 128.0f};
    SDL_RenderTextureTiled(renderer, this->sprite->texture, &tile_spr, 1.0f,
                           &tile_dst);
    SDL_SetRenderTarget(renderer, game->screen);
    ...
    for (int which_ant = 0; which_ant < NUM_ANTS; which_ant++) {
      ...
      game->push_sprite(this->ant->tex_id, this->ant->texture,
                        this->ant_src + which_ant, this->ant_dst + which_ant,
                        8);
    }
    ...
    game->push_sprite("", this->tunnel, &this->overlay_rect,
                      &this->overlay_rect, 0);
  ...
  game->push_sprite("", this->overlay, &this->overlay_rect, &this->overlay_rect,
                    2 * SCREEN_HEIGHT);
}
