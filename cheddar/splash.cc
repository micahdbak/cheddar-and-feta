...
Splash::Splash() {
  ...
  this->overlay_rect.h = THOOM_SCREEN_HEIGHT;

  this->animation = 0;

  this->sprite = new thoom::Sprite("sprites/splash.bmp", 128, 96, 250);
  ...
  this->ant = new thoom::Sprite("sprites/splash_ant.bmp", 32, 32, 100);

  this->initial_timer = thoom::game->ticks;
...
void Splash::step() {
  ...
  if (this->timer >= DISPLAY_FOR) {
    ...
    if (++this->animation >= this->animations) {
      thoom::game->map = "maps/init";  // leave the splash screen
      return;
    }
    ...
    alpha = THOOM_CLAMP((this->timer - (DISPLAY_FOR - 512)) / 2, 0, 255);
  } else if (this->timer < 512) {
    alpha = THOOM_CLAMP(256 - (this->timer / 2), 0, 255);
  }
  ...
  if (this->animation == 4) {
    ...
    for (int which_ant = 0; which_ant < NUM_ANTS; which_ant++) {
      ...
      thoom::game->push_sprite(this->ant->tex_id, this->ant->texture,
                               this->ant_src + which_ant,
                               this->ant_dst + which_ant, 8);
    }
    ...
    this->ched_dst.y =
        (perc * (float)THOOM_SCREEN_HEIGHT) - 16.0f + (6.0f * sin_res);
    ...
    thoom::game->push_sprite("", this->tunnel, &this->overlay_rect,
                             &this->overlay_rect, 0);
    thoom::game->push_sprite(this->sprite->tex_id, this->sprite->texture,
                             &this->ched_src, &this->ched_dst, 32);
    thoom::game->push_sprite(this->sprite->tex_id, this->sprite->texture,
                             &this->feta_src, &this->feta_dst, 32);
  } else {
    ...
    this->dst_rect.x = (float)(THOOM_SCREEN_WIDTH / 2) - 64.0f + shake_x;
    this->dst_rect.y = (float)(THOOM_SCREEN_HEIGHT / 2) - 64.0f + shake_y;

    thoom::game->push_sprite(this->sprite->tex_id, this->sprite->texture,
                             &this->sprite->frame, &this->dst_rect, 0);
  }
  thoom::game->push_sprite("", this->overlay, &this->overlay_rect,
                           &this->overlay_rect, 2 * THOOM_SCREEN_HEIGHT);
}
