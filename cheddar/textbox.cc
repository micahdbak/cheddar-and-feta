...
void Textbox::step() {
  ...
  if (local_controller.is_hit(Button::SELECT)) {
    ...
    this->i = j;
    game->draw_rect(game->ui, &this->text_rect, 24, 24, 24, 255,
                    SDL_BLENDMODE_NONE);
    game->draw_text(game->ui, this->running_text, this->font, this->text_rect.x,
  ...
  if (game->ticks - this->last_ticks > this->interval_ms) {
    ...
    if (c == '\n' || c == '\0') {
      ...
      SDL_FRect* prompt_src =
          game->fonts[this->font]->src_rect + CHAR_TEXTBOX_NEXT - ' ';
      SDL_FRect prompt_dst = {
          ...
          prompt_src->h};
      SDL_RenderTexture(renderer, game->fonts[this->font]->texture, prompt_src,
                        &prompt_dst);
