...
void Feta::step() {
    ...
    case FALSE:
        ...
        if (local_controller.is_hit(Button::ATTACK)) {
            ...
            case THROWABLE: {
                char buff[256];
                snprintf(buff, sizeof(buff), "%d,%d,%d,%d", (int)this->x, (int)this->y, x_dir, y_dir);
            ...
            case HELD_EFFECT: {
                ...
                char buff[256];
                snprintf(buff, sizeof(buff), "%s %d,%d", sel_item_id.c_str(), x_dir, y_dir);
        ...
        } else if (local_controller.is_hit(Button::TOSS) && sel_item_id != ITEM_NONE) {
            this->busy_ticks = game->ticks;
            ...
            // toss item
            NetSender::send_message(MSG_TOSS_ITEM, sel_item_id + " " + UseItem::Options(x, y, x_dir, y_dir));
            this->remove_item(sel_item_id);
