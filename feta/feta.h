...
private:
    ...
    enum Busy { FALSE, ATTACKING, ATTACKED, THROWING, EATING, DOWNED, FORCED_DANCE } is_busy = Feta::Busy::FALSE;
...
public:
    Object *create(const std::string &options) override {
        ...
        int fields = sscanf(options.c_str(), "%255[^,],%255[^,],%d %1023s", x_str, y_str, &animation, items_s);

        if (fields == 3) {
            items_s[0] = '\0';
        } else if (fields != 4) FATAL_ERROR

        return new Feta(str_to_float(x_str), str_to_float(y_str), animation, Mouse::read_items(items_s));
    }
};
