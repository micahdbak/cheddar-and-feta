...
class Init : public Object {
 public:
  Init() {
    this->ui = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA32,
...
class InitFactory : public ObjectFactory {
 public:
  ...
  Object* create(const std::string& options) override { return new Init(); }
};
...
void Init::step() {
  ...
  switch (net_state) {
    case NetworkAgent::State::NO_CONNECTION: {
      ...
      int x_dir = local_controller.is_hit(Button::RIGHT) -
                  local_controller.is_hit(Button::LEFT);
      int y_dir = local_controller.is_hit(Button::DOWN) -
                  local_controller.is_hit(Button::UP);
      ...
      if (this->render) {
        ...
        game->draw_text(this->ui, prompt, DEFAULT_FONT, 160 - prompt_w / 2, 70,
                        0);
        ...
        for (int i = 0; i < 36; i++) {
          ...
          SDL_RenderTexture(
              renderer,
              game->fonts[i == this->sel_c ? CODE_FONT : CODE_GRAY_FONT]
                  ->texture,
              &src_rect, &box_rect);
        ...
        if (valid_code) {
          ...
          game->draw_ui_box(this->ui,
                            ...
                            &enter_box);
          game->draw_text(this->ui, "OK",
                          36 == this->sel_c ? CODE_FONT : CODE_GRAY_FONT, 175,
                          148, 0);
        }
    ...
    default:
      if (this->render) {
        ...
        game->draw_text(this->ui, waiting, DEFAULT_FONT, 160 - waiting_w / 2,
                        116, 0);
      }

      // no connection after 15 seconds
      if (game->ticks - this->waiting_ticks > 15000) {
        net_agent->try_reset();
