#include "head.h"

SpitterHead::SpitterHead(Spitter *parent, std::shared_ptr<bool> deleted_ptr):
    parent(parent), deleted_ptr(deleted_ptr) {
    this->sprite = new Sprite("sprites/foe_spitter_head.bmp", 32, 32, 0);
    this->dst_rect.w = 32.0f;
    this->dst_rect.h = 32.0f;
    game->push_object(HURTBOX_OBJ, HurtBox::Options(this->id, -16, -16, 32, 32));
}

SpitterHead::~SpitterHead() {
    delete this->sprite;
}

void SpitterHead::step() {
    if (*this->deleted_ptr) {
        game->delete_object = true;
        return;
    }

    int x_dir, y_dir;
    dirs_from_direction(this->parent->displayed_direction, &x_dir, &y_dir);

    int distance = (int)(x_dir != 0 && y_dir != 0 ? 8.0f * DIAG_MULTIPLIER : 8.0f);
    this->x = this->parent->x + (float)(x_dir * distance);
    this->y = this->parent->y + (float)(y_dir * distance);

    this->dst_rect.x = this->x - 16.0f - game->corner_x;
    this->dst_rect.y = this->y - 16.0f - game->corner_y;

    this->sprite->set_animation(this->parent->displayed_direction);
    game->push_sprite(this->sprite->tex_id, this->sprite->texture, &this->sprite->frame, &this->dst_rect, 16);
}
