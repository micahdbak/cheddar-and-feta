...
TossedItem::TossedItem(float x, float y, int x_dir, int y_dir, bool from_feta, std::string item_id)
    : x(x), y(y), x_dir(x_dir), y_dir(y_dir), from_feta(from_feta), item_id(item_id) {
    std::string item_sprite_path = "sprites/" + item_id + ".bmp";
    this->sprite = new Sprite(item_sprite_path.c_str(), 16, 16, 0);
...
void TossedItem::step() {
    ...
    if (from_feta) {
        mouse_distance = distance_between_points(new_x, new_y, cheddar->x, cheddar->y);
    } else {
        mouse_distance = distance_between_points(new_x, new_y, feta->x, feta->y);
    }

    if (game->point_in_collider(new_x, new_y) || mouse_distance < 8.0f || game->ticks - this->spawned_ticks > 500) {
        game->delete_object = true;

        std::string dropped_obj_id = this->item_id + DROPPED_OBJ;
        game->push_object(dropped_obj_id, DroppedItem::Options(this->x, this->y));
