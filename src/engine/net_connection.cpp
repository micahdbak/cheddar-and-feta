...
void NetworkConnection::start_collecting(bool offerer) {
    if (this->state != NetworkConnection::State::DEAD) {
        std::cerr << "NetworkConnection::start_collecting fatal error: already collecting or alive" << std::endl;
        std::exit(1);
    }
    ...
    if (this->pc < 0) {
        std::cerr << "NetworkConnection::start_collecting fatal error: rtcCreatePeerConnection returned error code: " << this->pc << std::endl;
        exit(1);
    }
    ...
    if (offerer) {
        ...
        if (this->dc < 0) {
            std::cerr << "NetworkConnection::start_collecting fatal error: rtcCreateDataChannel returned error code: " << this->dc << std::endl;
            exit(1);
        }
...
void NetworkConnection::set_remote_description(const std::string &sdp, const std::string &type) {
    if (this->state != NetworkConnection::COLLECTING || this->pc < 0) {
        std::cerr << "NetworkConnection::set_remote_description fatal error: bad state" << std::endl;
        std::exit(1);
    }
...
void NetworkConnection::add_candidate(const std::string &cand, const std::string &mid) {
    if (this->state != NetworkConnection::COLLECTING || this->pc < 0) {
        std::cerr << "NetworkConnection::add_candidate fatal error: bad state" << std::endl;
        std::exit(1);
    }
...
void NetworkConnection::send(const std::string &message) {
    if (this->state != NetworkConnection::ALIVE || this->pc < 0) {
        std::cerr << "NetworkConnection::send warning: couldn't send message" << std::endl;
        return;
    }
...
void NetworkConnection::on_message(int, const char *message, int size, void *user_ptr) {
    NetworkConnection *connection = (NetworkConnection *)user_ptr;
    if (size > 0) {
        std::cerr << "NetworkConnection::on_message fatal error: received binary message" << std::endl;
        std::exit(1);
    }
