...
void NetworkSignaller::connect(const std::string &code, bool offerer) {
    if (this->state != NetworkSignaller::State::NOT_CONNECTED) {
        std::cerr << "NetworkSignaller::connect fatal error: already connecting or connected" << std::endl;
        std::exit(1);
    }
    ...
    if (this->ws < 0) {
        std::cerr << "NetworkSignaller::connect fatal error: rtcCreateWebSocket returned error code: " << this->ws << std::endl;
        std::exit(1);
    }
...
void NetworkSignaller::send(const std::string &message) {
    if (this->state != NetworkSignaller::State::CONNECTED || this->ws < 0) {
        std::cerr << "NetworkSignaller::send fatal error: not connected" << std::endl;
        std::exit(1);
    }
